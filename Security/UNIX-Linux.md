...
- i-node(index node)
  - 파일에 대한 정보를 기억하는 약 120bytes의 고정된 크기의 구조체
  - 구성요소
...
- PID 0(swapper)
  ...
  - init과 pagedaemon 프로세스를 만들기 위해 fork/exec를 두 번 실행
- PID 1(init)
...
- 항목
  - user_account
  - user_password
    - /etc/shadow 파일에 암호화되어 저장되어 있음
  - UID : User ID
    - 보통 100번 이하는 시스템이 사용, 0번은 시스템 관리자를 나타냄
  - GID : Group ID
  - comment
  - home_directory
    - 로그인 성공 후에 사용자가 위치할 홈 디렉터리의 절대경로
  - login_shell: 로그인 셸의 절대경로
    - 로그인이 불필요한 계정의 셸은 `/sbin/nologin`, `/bin/false`으로 지정
...
- 항목
  - user_id
  - encryption_pw: 일방향 해시 알고리즘을 이용해 암호화한 패스워드
...
- SetUID
  - 실제 사용자가 아닌 해당 파일의 소유자 권한으로 실행
  - `/etc/passwd` 파일에 적용
- SetGID
  - 실제 사용자 계정이 아닌 소유그룹의 권한으로 실행
  - 부서원의 권한을 일괄적으로 적용할 때 사용
- sticky-bit
  - 디렉터리에 특별한 접근권한 부여(공유모드)
